# -*- coding: utf-8 -*-
"""Q20 每日净值跟踪：拉腾讯行情 → 更新净值序列 → 重新生成报告。
...
def main():
    ...
    if not port or not port.get("positions"):
        print("无持仓，请先运行 _sim_q20_init.py")
        return
    tcodes = [p["tcode"] for p in port["positions"]] + [Q.BM_CODE]
    ...
    mv, nav, missing = Q.port_value(port, q)
    if missing:
    ...
    nav_list = Q.upsert_nav(nav_list, entry)
    ...
    print(f"{date} 净值 {nav:,.2f} | 当日 {dd} | 累计 {cum:+.2%} | "
          f"{Q.BM_NAME} {entry['bm_cum']:+.2%} | 超额 {exc:+.2%}")
