...
def main():
    ...
    for tk, d in px.items():
        ...
        for dt, r in d.items():
            ...
            if c and c > 0 and dt >= "2021-05-01":
                mm[dt] = BarData(tk, dt, r.get("open") or c,
                                 r.get("high") or c, r.get("low") or c, c)
                n_adj += 1
        if mm:
            bars[tk] = mm
    print(f"未复权 bar: {len(bars)} 只 | {n_adj} 条（adj 折算 OHLC）")
    ...
    out = {"results": results, "idx": {"total": idx_total, "ann": idx_ann,
           ...
           "periods": PIT_DATES}
    json.dump(out, open("_bt_daily_results_unadj.json", "w", encoding="utf-8"),
