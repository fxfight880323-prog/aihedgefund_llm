...
# 可通过命令行 --candidate-dates YYYY-MM-DD,YYYY-MM-DD 覆盖（pipeline/dates.py 自动生成最近工作日）
CANDIDATE_DATES = ["2026-08-27", "2026-08-26", "2026-08-25", "2026-08-24", "2026-08-21"]
...
def main():
    ...
    # --candidate-dates 覆盖内置候选（pipeline 自动注入最近工作日）
    dates = CANDIDATE_DATES
    for i, a in enumerate(sys.argv):
        if a == "--candidate-dates" and i + 1 < len(sys.argv):
            dates = [x.strip() for x in sys.argv[i + 1].split(",") if x.strip()]
            print(f"候选日期由命令行注入: {dates}")
    ...
    for d in dates:
        ...
        time.sleep(2)
    if not fac_asof:
        cached = load_cache(FAC_FILE)
        if cached.get("records"):
            fac_asof = cached["as_of"]
            print(f"!! 无健康因子日，回退缓存健康日 {fac_asof}（基本面因子前向填充，可接受）")
        else:
            print("!! 无健康因子日期且无缓存，中止")
            sys.exit(1)
