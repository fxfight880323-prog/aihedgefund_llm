# -*- coding: utf-8 -*-
"""Q20 半年调仓：把持仓重置为新名单（默认 _bt_q20_kfin.json final）等权。

流程：先重跑名单管线（_bt_q20_kfin.py，刷新 as_of 为最新交易日），再运行本脚本。
逻辑：卖出剔除股 → 以调仓时净值等权重置（买入新增 + 保留股权重回补/削减），
15bp 单边成本，全部记录流水，更新净值与报告。
用法:
  python _sim_q20_rebalance.py            # 正式调仓
  python _sim_q20_rebalance.py --dry      # 只打印调仓计划不落盘
  python _sim_q20_rebalance.py --list _bt_q20_kfin.json
"""
import argparse
import os
import sys
...
def main():
    ...
    ap.add_argument("--list", default="_bt_q20_kfin.json")
    ap.add_argument("--dry", action="store_true")
    args = ap.parse_args()
    list_file = os.path.join(Q.BASE, args.list)

    data = Q.jload(list_file)
    assert data and data.get("final"), f"读不到名单 {args.list}"
    port = Q.jload(Q.PORT_FILE)
    assert port and port.get("positions"), "无持仓，请先 _sim_q20_init.py"

    stocks = data["final"]
    n_new = len(stocks)
    new_map = {Q.tx_code(s["code"]): s for s in stocks}
    old_map = {p["tcode"]: p for p in port["positions"]}

    tcodes = list(new_map) + [t for t in old_map if t not in new_map] + [Q.BM_CODE]
    ...
    for tc, s in new_map.items():
        ...
        if delta > 1:
            ...
            trades.append({"date": date, "type": "调仓卖出", "code": s["code"], "name": q[tc]["name"],
                           ...
                           "fee": round(amount * Q.FEE, 2), "reason": "调仓:权重削减"})
            shares -= shares_cut
            n_sell_adj += 1
    ...
    mv, nav, _ = Q.port_value({"positions": positions, "cash": port["cash"]}, q)
    nav_list = Q.jload(Q.NAV_FILE, []) or []
    ...
    entry = {
        ...
        "cum_ret": round(nav / Q.CAPITAL - 1, 6),
    ...
    print(f"调仓后: 现金 {port['cash']:,.2f} | 净值 {nav:,.2f} ({nav / Q.CAPITAL - 1:+.2%})")
    ...
    port["config"]["next_rebalance"] = Q.next_rebalance(date)
    Q.jdump(port, Q.PORT_FILE)
    Q.jdump(trades, Q.TRADE_FILE)
    Q.jdump(Q.upsert_nav(nav_list, entry), Q.NAV_FILE)
    R.gen_report(quotes=q)
    print(f"已落盘并更新报告 _sim_q20_report.html | 下次调仓 {port['config']['next_rebalance']}")
