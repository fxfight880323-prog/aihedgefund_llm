# -*- coding: utf-8 -*-
"""Growth Loop v4：估值卖出改用一致预期 PEG 触发（预期恶化即卖）。

背景（v3 结论）：v3rec = 买入端分析师一致预期过滤 + 风格开关 + PS/PS_med≥2
估值卖出 → +27.0%。用户要求把估值卖出的触发信号从「PS 历史分位」换成
「一致预期 PEG / 预期修正」（响应更快：分析师预期领先价格分位）。

诚实前提：_gl_v3_consensus.json 只有 10 个调仓月的 PIT 快照，非调仓月
没有一致预期历史数据 → PEG/rev 卖出只能调仓月 overlay 触发，无月度出场
（PS 卖出保留了月度出场）。且 v3rec 买入端已硬过滤 peg<2 / rev4w>0 →
卖出端查同字段在同一时点必然冗余。故拆两个正交问题：

  问题 1：PEG/预期恶化卖出本身有没有用？
    → 买入端不滤 consensus（v2rec 买入），卖出端 PEG 全权负责
    v4a_peg  v2买入 + PEG≥2.0 清仓（调仓 overlay）
    v4a_rev  v2买入 + rev4w<0 清仓（预期恶化即卖）
    v4a_both v2买入 + 任一触发
  问题 2：v3rec 买入已过滤的前提下，PEG 卖出替代/叠加 PS 卖出？
    v4b_swap v3rec买入 + peg_rev 卖出【替换 PS 卖出 = 用户字面需求】
    v4b_plus v3rec买入 + ps 卖出 + peg_rev 叠加

对照：v2rec（+0.7%）、v3rec（+27.0%）。
数据均 PIT（调仓月快照），无未来函数。

用法: python _gl_v4_variant.py <mode>
"""
...
# ---------------------------------------------------------------------------
# 估值卖出纪律策略（v4：卖出信号 = PS 分位 / 一致预期 PEG / 预期修正 可切换）
# ---------------------------------------------------------------------------

class ValSellStrategy(StrategyTemplate):
    """卖出纪律。sell_mode:
      ps      → PS/PS_med≥2（调仓 overlay + 非调仓月逐月出场，v3rec 原逻辑）
      peg     → 一致预期 PEG≥peg_th 清仓（仅调仓月 overlay，无月度数据）
      rev     → 预期修正 rev4w<0 清仓（仅调仓月 overlay，无月度数据）
      peg_rev → 任一触发（仅调仓月 overlay）
      ps_pegrev → PS 卖出（含月度出场）+ PEG/rev 卖出（调仓 overlay）叠加
    PEG 系诚实限制：非调仓月无一致预期历史快照 → 不触发；consensus 缺失
    → 放行不误杀（与 valguard_na 口径一致）。"""
    ...
    def __init__(self, engine, setting):
        ...
        self.sell_mode = setting.get("sell_mode", "ps")
        self.consensus = setting.get("consensus", {})
        self.peg_th = setting.get("peg_th", PEG_MAX)
        self.sell_log: list[dict] = []
        self.history: list[dict] = []
        self.na_check = 0          # consensus 缺失放行计数（诚实标注覆盖率）

    def _ps_sell(self, tk: str, dt: str) -> tuple[bool, str]:
    ...
    def _peg_sell(self, tk: str, dt: str) -> tuple[bool, str]:
        """一致预期 PEG / 预期修正触发（仅调仓月有 PIT 快照）。"""
        if dt not in self.rebalance_dts:
            return False, ""
        row = (self.consensus.get(dt) or {}).get(tk)
        if not row:
            self.na_check += 1
            return False, ""
        hits = []
        if self.sell_mode in ("peg", "peg_rev", "ps_pegrev"):
            p = row.get("peg")
            if p is not None and p >= self.peg_th:
                hits.append(f"PEG={p:.2f}≥{self.peg_th}")
        if self.sell_mode in ("rev", "peg_rev", "ps_pegrev"):
            r = row.get("rev4w")
            if r is not None and r < 0:
                hits.append(f"rev4w={r:.1f}<0")
        return bool(hits), " + ".join(hits)

    def _sell_check(self, tk: str, dt: str) -> tuple[bool, str]:
        if self.sell_mode == "ps":
            return self._ps_sell(tk, dt)
        if self.sell_mode in ("peg", "rev", "peg_rev"):
            return self._peg_sell(tk, dt)
        if self.sell_mode == "ps_pegrev":
            hit, reason = self._ps_sell(tk, dt)
            if hit:
                return True, reason
            return self._peg_sell(tk, dt)
        return False, ""
    ...
    def on_bars(self, bars):
        dt = self.engine.datetime
        if dt not in self.rebalance_dts:
            if self.sell_mode in ("ps", "ps_pegrev"):
                self._maybe_sell(dt, bars)   # PS 卖出有月度出场；PEG 系无月度数据
            return
...
def main():
    mode = sys.argv[1] if len(sys.argv) > 1 else "v3rec"
    use_yoy300 = False
    use_valguard = False
    use_styleswitch = mode in ("v2rec", "v3rec", "v4a_peg", "v4a_rev",
                               "v4a_both", "v4b_swap", "v4b_plus")
    use_deceldw = False
    use_consensus = mode in ("v3rec", "v4b_swap", "v4b_plus")   # 买入端过滤
    second_deriv = ("boost" if use_consensus else "none")
    sell_mode = {"v2rec": "ps", "v3rec": "ps", "v4a_peg": "peg",
                 "v4a_rev": "rev", "v4a_both": "peg_rev",
                 "v4b_swap": "peg_rev", "v4b_plus": "ps_pegrev"}[mode]
    peg_th = 2.0
    nav_file = f"_bt_gl_v4_{mode}_nav.json"
    det_file = f"_bt_gl_v4_{mode}_detail.json"
    diag_file = f"_bt_gl_v4_{mode}_diag.json"
    ...
    print(f"② [{mode}] 逐期筛选 "
          f"(switch={use_styleswitch} consensus买入={use_consensus} "
          f"2nd_deriv={second_deriv} sell={sell_mode} peg_th={peg_th})")
    ...
    strategy = ValSellStrategy(engine, {
        ...
        "sell_mode": sell_mode, "consensus": consensus, "peg_th": peg_th})
    ...
    if sells:
        ...
        print(f"\n⑤' 卖出纪律[{sell_mode}]: {len(sells)} 笔"
              f"（调仓 overlay {n_overlay} / 月度出场 {n_monthly} | "
              f"PEG 检查缺失放行 {strategy.na_check} 次）")
    ...
    json.dump({"mode": mode, "mdd": mdd, "annualized": ann,
               ...
               "flags": {"yoy300": use_yoy300, "valguard": use_valguard,
                         ...
                         "sell_mode": sell_mode,
                         "consensus_buy": use_consensus,
                         "second_deriv": second_deriv},
               "diag": diag, "per_period_ret": per_ret,
               "n_sells": len(sells), "na_check": strategy.na_check},
