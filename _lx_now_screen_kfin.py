"""刘旭式框架当前时点筛选 — KEEP-FIN 版（不剔除金融/银行，对应回测 core 口径 +55.55%）。
...
与 _lx_now_screen.py 唯一差异: 保留银行/非银金融股（对应回测 core 变体
+55.55%/MDD -19.13%；银行贡献 76% 收益，见 _bank_attr_report.html 归因）。
...
OUT_JSON = "_lx_now_results_kfin.json"
OUT_CSV = "刘旭框架_当前推荐A股_含金融.csv"
OUT_HTML = "_lx_now_report_kfin.html"
...
def main():
    ...
    # ---- KEEP-FIN：不剔除金融（本变体目的，对应回测 core 口径）----
    fin_set = load_fin_set()   # 仅用于行业标记
    print(f"KEEP-FIN 模式: 保留金融股（{len(fin_set)} 只标记不剔除）")
