# -*- coding: utf-8 -*-
"""生成 _sim_q20_report.html：净值曲线 SVG + 持仓明细 + 交易流水 + 口径说明。"""
import datetime
import os
import sys
...
.badge.avoid{background:#fdecea;color:#c0392b;}
...
def _chart(nav, reb_dates):
    ...
    nav_idx = [e["nav"] / Q.CAPITAL for e in nav]
...
def gen_report(quotes=None):
    port = Q.jload(Q.PORT_FILE)
    if not port:
        return
    nav = Q.jload(Q.NAV_FILE, []) or []
    trades = Q.jload(Q.TRADE_FILE, []) or []
    cfg = port["config"]
    if quotes is None:
        quotes = Q.fetch_quotes([p["tcode"] for p in port["positions"]] + [Q.BM_CODE])
    bm = quotes.get(Q.BM_CODE, {})
    ...
    for i, (p, price, pct, mv, pnl, pnl_pct) in enumerate(rows, 1):
        ...
        flag_h = ""
        if flag == "规避":
            flag_h = "<span class='badge avoid'>规避</span>"
        elif flag == "确认":
            flag_h = "<span class='badge ok'>确认</span>"
        elif flag and flag != "中性":
            flag_h = f"<span class='badge'>{flag}</span>"
        qs = p.get("q_score")
        bl = p.get("blend")
...
<div class="sub">建仓 {_d(cfg['created'])} ｜ 初始资金 {cfg['capital']:,.0f} 元 ｜ 名单 as_of {_d(cfg.get('list_as_of', ''))}
<br>方法：{cfg.get('list_method', '')}
<br>池子：{cfg.get('pool', '')}
<br>调仓 {cfg['rebalance']}，下次 <b>{cfg.get('next_rebalance', '')}</b> ｜ 基准 {cfg['benchmark']} ｜ 行情 {cfg.get('quote_source', '')}</div>

<div class="cards">
<div class="card"><div class="k">最新净值</div><div class="v">{navv:,.0f}</div><div class="s">指数 {navv / Q.CAPITAL:.4f}</div></div>
...
· <b>名单</b>：_bt_q20_kfin.json final —— q20 混合排序（0.8×PE便宜度 + 0.2×质量分，池内百分位）top40；
  池子 = 万得全A PIT <b>含金融</b> core-pass（KFIN 221 只 @{_d(cfg.get('list_as_of', ''))}）；
  band 规避层：PB 5年分位&gt;90% 剔除、q20 替补池顺位递补（本期剔除江苏银行 → 递补正泰电器）。<br>
...
· <b>回测背书</b>：q20 +3.40pp 增量验证于剔金融口径（core_finex，2021-08~2026-04 日频复权）；含金融口径增量未单独回测，本组合为 q20 排序层在核心管线（core +55.55%/MDD -19.13%）上的落地展示。银行占比约 40%，收益来源偏金融价值修复。<br>
· <b>镜像</b>：{cfg.get('mirror', '')}；净值与调仓以本地账本（_sim_q20_portfolio.json）为权威。<br>
· <b>引擎</b>：_sim_q20_engine.py（行情/账本）· _sim_q20_init.py（建仓）· _sim_q20_daily.py（每日净值）· _sim_q20_rebalance.py（半年调仓）· 每日 15:35 自动化运行 _sim_q20_daily.py。
</div></div>
